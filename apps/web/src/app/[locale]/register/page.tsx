...
const ROLES = [
  { id: 'buy', label: 'Buy / Sell bikes', icon: '🛒', desc: 'Browse & list bikes for sale' },
  { id: 'rent', label: 'Rent bikes', icon: '🚲', desc: 'Rent bikes for short trips' },
  { id: 'repair', label: 'Repair bikes', icon: '🔧', desc: 'Offer or find repair services' },
]
...
export default function RegisterPage() {
  ...
  const onSubmit = async (data: RegisterFormData) => {
    ...
    login(data.email)
    toast.success('Account created! Welcome to BiciMarket!')
    ...
    <div className="min-h-screen bg-gradient-to-br from-gray-50 via-white to-orange-50 dark:from-secondary-900 dark:via-secondary-800 dark:to-secondary-900 flex items-center justify-center py-12 px-4">
      <motion.div initial="hidden" animate="visible" className="w-full max-w-md">
        ...
        >
              ...
              {passwordValue && (
                <div className="mt-2">
                    ...
                    {[1, 2, 3, 4, 5].map((i) => (
                      <div
                        ...
                        className={clsx(
                          ...
                          i <= strength.score ? strength.color : 'bg-gray-200 dark:bg-secondary-600'
                        )}
                      />
                    ))}
                  </div>
                  <p className={clsx('text-xs mt-1 font-medium', {
                    ...
                    'text-yellow-500': strength.label === 'Fair',
                      ...
                      return (
                        ...
                        >
                          ...
                          <div className="flex-1">
                            <div className="text-sm font-semibold text-gray-800 dark:text-white">{role.label}</div>
                            <div className="text-xs text-gray-500 dark:text-gray-400">{role.desc}</div>
                          </div>
                          {selected ? (
                            <CheckCircle2 className="w-5 h-5 text-primary flex-shrink-0" />
                          ) : (
                            <Circle className="w-5 h-5 text-gray-300 dark:text-secondary-500 flex-shrink-0" />
                          )}
                        </button>
                      )
                    })}
            ...
            <motion.div custom={7} variants={fadeIn}>
              <label className="flex items-start gap-3 cursor-pointer">
                <input
                  {...register('terms')}
                  type="checkbox"
                  className="mt-0.5 w-4 h-4 rounded border-gray-300 text-primary focus:ring-primary/50 accent-primary"
                />
          ...
          {/* Divider */}
          <motion.div custom={9} variants={fadeIn} className="my-5 flex items-center gap-3">
            <div className="flex-1 h-px bg-gray-200 dark:bg-secondary-600" />
            <span className="text-xs text-gray-400 dark:text-gray-500 font-medium">or sign up with</span>
            <div className="flex-1 h-px bg-gray-200 dark:bg-secondary-600" />
          </motion.div>

          <motion.div custom={10} variants={fadeIn} className="grid grid-cols-2 gap-3">
            <button
              type="button"
          ...
          <motion.p custom={11} variants={fadeIn} className="text-center text-sm text-gray-500 dark:text-gray-400 mt-6">
            Already have an account?{' '}
            <Link href={`${localePath}/login`} className="text-primary font-semibold hover:text-primary-600">
              Sign in
            </Link>
          </motion.p>
        </motion.div>
