...
function StepCard({ column, index }: { column: typeof columns[0]; index: number }) {
  ...
  return (
    <motion.div
      ...
      className="relative"
    >
      <div className="bg-white/5 backdrop-blur-sm border border-white/10 rounded-2xl p-8 hover:border-white/20 transition-all duration-300 h-full group hover:bg-white/8">
        {/* Icon circle */}
        ...
        <p className="text-white/50 text-sm mb-8">{column.subtitle}</p>
        ...
        <div className="space-y-5">
          {column.steps.map((step, i) => (
            ...
            >
              ...
              <p className="text-white/70 text-sm leading-relaxed pt-1">{step.text}</p>
            </motion.div>
          ))}
        </div>
        ...
        {/* CTA */}
        <motion.a
          href={`/${column.title.toLowerCase()}`}
          initial={{ opacity: 0 }}
          animate={isInView ? { opacity: 1 } : {}}
          transition={{ duration: 0.4, delay: 0.7 }}
          className={`mt-8 inline-flex items-center gap-2 text-sm font-semibold bg-gradient-to-r ${column.accent} bg-clip-text text-transparent hover:opacity-80 transition-opacity`}
        >
          Get started →
        </motion.a>
      </div>
...
export function HowItWorks() {
  ...
  return (
    <section ref={sectionRef} className="py-24 bg-secondary-900 relative overflow-hidden">
      {/* Background gradient */}
      ...
      <div className="container mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 lg:gap-8">
          {columns.map((col, i) => (
            <StepCard key={col.title} column={col} index={i} />
          ))}
        </div>
        ...
        >
          <a
            href="/how-it-works"
            className="inline-flex items-center gap-3 border-2 border-primary-500/50 hover:border-primary-500 text-primary-400 hover:text-primary-300 font-bold px-8 py-4 rounded-2xl transition-all duration-200 hover:bg-primary-500/10"
          >
